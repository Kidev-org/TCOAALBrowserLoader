...
const BUILTIN_MOD_ASSETS = [
  ...
  "/mods/_seamlessMaps/www/img/icon.png",
  "/mods/_seamlessMaps/www/js/plugins/SeamlessMaps.js",
  "/mods/_interactGlint/www/img/icon.png",
  "/mods/_interactGlint/www/js/plugins/InteractGlint.js",
...
/**
 * Bytes of an IDB value whatever shape it was stored in. The loader writes
 * ArrayBuffers, but older imports and the cross-origin migration have left
 * Blobs and typed arrays behind, and `new Uint8Array(blob)` is an EMPTY array,
 * not an error: the language paths below used to read such a CLD as "no
 * signature" and answer /lang-data.json with a 404. Strings come back UTF-8.
 */
...
/**
 * Parse language data out of a mod's own language file: a LANGDATA-prefixed
 * CLD (TLCOAAA's data/dialogues, stored plain), a .loc (TCOAAR: JSON behind a
 * run of spaces) or bare JSON. Anything before the first "{" is skipped, so
 * both prefixes and the padding are handled by one rule. Returns the parsed
 * object when it carries at least one of the text dicts, else null.
 */
async function parseModLangBytes(value, keyForDecrypt) {
  const bytes = await assetBytes(value);
  if (!bytes || !bytes.length) return null;
  const plain = dekit(bytes, keyForDecrypt);
...
/** The mods.json entry for `modId`, from the SW's durable copy; null if unknown. */
async function catalogEntry(db, modId) {
  try {
    const text = await getAsset(db, SHELL_IDB_FALLBACK_KEYS["mods.json"]);
...
async function loadBaseCLD(db) {
  ...
  const cldBytes = await assetBytes(await getAsset(db, CLD_KEY));
  if (cldBytes !== null) {
    const bytes = dekit(cldBytes, CLD_KEY);
 ...
 *
 * The pre-extracted cache first; then the mod's own language file at the
 * path its mods.json entry names (langFile: TLCOAAA's data/dialogues, TCOAAR's
 * languages/english/dialogue.loc, ...), and the two historical layouts for a
 * mod the catalog does not describe. The cache is a convenience, not the
 * source of truth: it is written by lang-shim at install time only, so a boot
 * repair that refetched the language file, or any wipe that took the cache
 * but not the file, used to leave an active overhaul with no language data
 * at all - /lang-data.json then fell back to the base CLD, and where that was
 * unreadable too the DRM read a 404 body as its language file and crashed
 * with "Default language missing or broken". Whatever is parsed here from the
 * file is written back to the cache so the next boot is the fast path again.
 */
async function loadModLangData(db, modId) {
  ...
  for (const rel of candidates) {
    const raw = await getAsset(db, "mod:" + modId + ":" + rel);
    if (raw === null) continue;
    const parsed = await parseModLangBytes(raw, rel);
    if (!parsed) {
      console.warn("[sw] Failed to parse mod language file:", modId, rel);
      continue;
    }
    // Heal the cache so the next boot takes the fast path. Best effort.
    putAsset(db, cachedKey, JSON.stringify(parsed)).catch(() => {});
    return parsed;
  }
