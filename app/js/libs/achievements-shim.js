 ...
 *
 * Base game only: with an overhaul active every entry point here is inert (see
 * isBaseContext), so a mod's own achievement system is what the player gets.
 *
 * Exposes:
...
(function () {
  ...
  // Base game only. Achievements are a base-game feature (the DRM's
  // ending/gallery system): an overhaul rewrites the dataset, and may ship an
  // achievement system of its own (TLCOAAA adds a title command in its payload
  // and a Scene_Achievements in Achievements.js). So with an overhaul active
  // this whole module is inert: no title entry, no scene, no unlock, no toast,
  // no stats report - the mod's inherited `achv` events (TCOAAR, TCOAAJ) must
  // not unlock base progress from inside a mod, and whatever the mod defines
  // under the same names must stay the mod's. The base game and its
  ...
  function applyPatches() {
    ...
    // Under an overhaul, touch nothing: Scene_Achievements, the title
    // "achievements" handler, MenuOptions' icon and the command window width
    // may all be the mod's own (see isBaseContext). Only the stats backlog is
    // flushed, since it describes base unlocks made earlier and is
    // context-independent.
    if (!isBaseContext()) {
      reportUnlocks();
      return;
