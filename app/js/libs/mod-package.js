...
/*
 * Minimal ZIP reader/writer and the tcoaal-mod/4 packager, used by
 ...
 */
(function (root) {
  var FORMAT = "tcoaal-mod/4";
  // /4 grew two things over /3: a "copy" entry (the player's own file laid
  // down under the mod's name, carrying no bytes) and a "from" field on
  // patch entries (replay on that base file, land at rel). A /3 package uses
  // neither, so it is a /4 package as it stands and is still read.
  var READABLE = ["tcoaal-mod/3", FORMAT];
  ...
  function readme(m) {
    ...
    lines.push("");
    lines.push("This file contains no content from the base game: files the game");
    lines.push("already has are referenced, not carried, and changed data files");
    lines.push("travel as patches. Install it with the TCOAAL Mod Loader.");
    return new TextEncoder().encode(lines.join("\n") + "\n");
  }
  ...
  async function parse(bytes) {
    ...
    // Exact match, not a "tcoaal-" prefix test: a future tcoaal-mod/5 would
