 ...
 */
/*
 * InteractGlint: a small, periodic glint on every interactable on the map,
 * the way the old Resident Evil games sparkled an item you could pick up.
 *
 * What counts as interactable is exactly what the game's own interact
 * balloon (Hint.process in the DRM payload) would open on: an event whose
 * active page has the Action Button trigger and whose payload-defined
 * isEnabled() rule passes (the "enabled: <switch|var|item> ..." comment on
 * the page's first line, which is how the story retires an object once it
 * has been used). Player Touch events (room exits) are left alone.
 *
 * Most of the game's interactables are events with NO graphic, placed over
 * the parallax art, so there is no sprite to brighten. The glint is its own
 * sprite: a soft glow with a thin four-point star, drawn additively at the
 * center of the event's hitbox (OrangeEventHitboxes) or tile. It is
 * invisible for most of a PERIOD, then twinkles for TWINKLE seconds: alpha
 * rises to PEAK_ALPHA and back with a slight scale pulse and a quarter turn.
 * Each event starts its cycle at a phase derived from its id so a room full
 * of objects does not blink in lockstep.
 *
 * The layer is a child of the map's tilemap, at the z the game uses for its
 * balloons and animations so it clears OrangeOverlay's foreground parallax,
 * and scrolling and SRD_CameraCore's zoom apply to it for free. It hides entirely while an event or message is running, and
 * whenever the game's own "UI Hints" option is off, since that option is
 * the player's word on hints of this kind.
 *
 * The pure parts (qualifies, phaseFor, envelope, anchorFor, shouldShow) are
 * exposed on the InteractGlint namespace for tools/test.js.
 */
...
(function () {
  ...
  // Tilemap z. Characters end at 5 and the stock engine tops out at 9
  // (Sprite_Destination), but OrangeOverlay parks the depth-illusion
  // parallax at 20, with shadow 21, fog 22 and light 23 above it, and a
  // glint under that layer draws behind the very furniture it marks. The
  // game lifts its balloons and animations to 30 to clear those; the
  // glint layer sits with them.
  var LAYER_Z = 30;

  // Pure parts

  // The page has something to run: not empty, not just the terminating
  // "end" command. This is what Hint's isEnabled() checks before its
  // comment rule, and it is the whole rule when the payload is absent.
  ...
  // A fixed fraction of the period per event id, spread with a Weyl
  // sequence (golden ratio) so neighbouring ids land far apart.
  function phaseFor(id) {
    var p = (id * 0.6180339887498949) % 1;
    return p < 0 ? p + 1 : p;
  }

  // Alpha and scale at time t (seconds) for an event at the given phase.
  ...
  // Tile-space center of the event's hitbox, or of its tile when it has
  // none (OrangeEventHitboxes not loaded, or default 1x1 box).
  ...
  // Bitmap: a radial glow and a thin four-point star, white with a warm
  // tint, meant to be drawn with additive blending at low alpha.
  ...
  function glintBitmap() {
    ...
    if (ctx) {
      ...
      // Star: two thin diamonds, one per axis, fading toward their tips.
      var arm = c - 1;
      var half = 1.2;
  ...
  // Sprite_GlintLayer: one child of the tilemap holding one glint sprite
  // per qualifying event, pooled by event id.
  ...
  Sprite_GlintLayer.prototype.update = function () {
    ...
    var show = shouldShow({
      inputHint: !!(typeof ConfigManager !== "undefined" && ConfigManager.inputHint),
  ...
  // Tilemap.update calls update() on every child, which is what drives the
  // layer; nothing else needs hooking.
