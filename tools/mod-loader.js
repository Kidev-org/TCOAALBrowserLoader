...
/*
 * The mod loader engine: install `tcoaal-mod/4` packages (and the /3 ones
 * before it) beside a game and play them WITHOUT ever writing inside the
 * game's own files.
 *
...
const FORMAT = "tcoaal-mod/4"; // what this build writes; parse also reads /3
...
async function unpack(opts) {
  ...
  result({
    ...
    // Where a newer version of THIS mod lives, as the installer carries it.
    // The app passes it back to --check-update as --source for a mod the
    // player's folder does not have (or has without a source of its own).
    updateSource: updateSourceOf(manifest, null),
    dir: opts.out,
  });
}
 ...
 * refuses to start unless it matches a constant compiled into its DRM
 * payload, with "Game files corrupted. Re-installation may repair files."
 ...
 * in www whose name ends in .txt or .url is restored from the player's own
 * copy: the shipped copyright, credits and EULA documents, nothing the game
 * reads as content. Matching by shape rather than by one hardcoded name keeps
...
async function applyModIntoProfile(game, dir, id) {
  ...
  for (const f of variant.files || []) {
    ...
    if (f.type === "verbatim") {
    ...
    } else if (f.type === "patch" || f.type === "copy") {
      // Both start from a file the player already has: f.from where the
      // entry names one (a mod authored as a project keeps data/Actors.json
      // where the game stores data/be1a37535e921f91, and the package refers
      // to the player's copy rather than carrying it), otherwise this path.
      ...
      if (f.from && !safeRel(f.from)) {
        fail(`Refusing an unsafe path in "${id}": ${f.from}`);
      }
      const srcAbs = path.join(wwwOut, src);
      if (!fs.existsSync(srcAbs)) {
        fail(
          `"${id}" needs ${src} for ${f.rel}: it is missing from your game.\n` +
            `Your copy does not match the build this mod was made for.`,
        );
      }
      const live = new Uint8Array(fs.readFileSync(srcAbs));
      const srcPlain = C.dekit(live, src);
      if (f.type === "copy") {
        plain = srcPlain;
        counts.copied++;
      } else {
        const doc = JSON.parse(new TextDecoder().decode(srcPlain));
...
/**
 * Where newer versions of a mod live.
 *
 * `update` is the author's own answer and wins. Failing that, `online` is
 * taken as one: a stamped ONLINE installer carries a source naming where the
 * mod is published, and "where the mod is published" and "where a newer
 * version of it is published" are the same place. Without the fallback an
 * online installer, the one shape that always knows a release feed, was the
 * one shape that could never report an update.
 *
 * `extra` is a source the caller supplies (the installer's own carried
 * manifest, for a mod whose installed copy names none) and is the last resort.
 */
function updateSourceOf(manifest, extra) {
  const m = manifest || {};
  if (m.update && (m.update.url || m.update.github || m.update.manifest)) {
    return m.update;
  }
  if (m.online && (m.online.url || m.online.github || m.online.manifest)) {
    return m.online;
  }
  const e = extra || null;
  if (e && (e.url || e.github || e.manifest)) return e;
  return null;
}

async function latestFor(manifest, extra) {
  return resolveSource(updateSourceOf(manifest, extra));
}

/**
 * The version an update is measured against.
 *
 * Two things can claim to be "the version the player has" and the newest of
 * them is the honest answer, because neither can be trusted alone:
 *
 *   - what is installed in addons/. For an ONLINE installer this is the real
 *     answer (the tag of the release that was actually downloaded), and the
 *     placeholder inside the binary says nothing useful.
 *   - what the installer carries. For an OFFLINE installer this is the real
 *     answer, and the app passes it as --current: the mod may not be installed
 *     yet, and an installer stamped with a newer mod than the folder holds is
 *     not out of date.
 *
 * Taking the newer of the two is what keeps the check monotone: an update is
 * proposed only for something genuinely newer than anything already on hand,
 * so a player never gets offered the version they are already running.
 */
function baselineVersion(installed, carried) {
  const a = String(installed || "");
  const b = String(carried || "");
  if (!a) return b;
  if (!b) return a;
  return compareVersions(a, b) < 0 ? b : a;
}
...
async function checkUpdate(opts) {
  ...
  const extra = opts.source || null;
  const out = [];
  for (const id of ids) {
    const manifest = readManifestOf(game, id);
    // --source lets the app check a mod the folder does not have yet: the
    // installer knows where its own mod is published before it is installed.
    if (!manifest && !extra) continue;
    const m = manifest || {};
    const source = updateSourceOf(m, opts.id === id ? extra : null);
    if (!source) continue;
    const current = baselineVersion(
      m.version,
      opts.id === id ? opts.current : "",
    );
    try {
      const latest = await resolveSource(source);
      if (!latest) continue;
      out.push({
        id,
        current: current,
        ...
        via: latest.via,
        installed: !!manifest,
        newer: compareVersions(current, latest.version) < 0,
      });
    } catch (e) {
...
async function applyUpdate(opts) {
  ...
  const latest = await latestFor(manifest, opts.source);
  ...
  const tmp = await downloadPackage(
    ...
    latest.url,
    `update-${opts.id}`,
    `Downloading ${manifest.name || opts.id} ${latest.version}...`,
  );
  try {
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--status") opts.mode = "status";
      ...
      // A JSON {url|github|manifest}, used when the installed copy names no
      // update source of its own. Parsed here so a malformed one fails with a
      // sentence rather than deep inside resolveSource.
      const raw = argv[++i];
      try {
        opts.source = JSON.parse(raw);
      } catch (e) {
