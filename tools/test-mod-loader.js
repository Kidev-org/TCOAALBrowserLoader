...
(async function main() {
  ...
  // Updates.
  //
  // The check is what tells a player a new version exists, and it used to
  // report nothing for anybody: it only looked at `update`, only for a mod
  // already installed, and only ever measured against the copy in addons/.
  {
    const upGame = path.join(tmp, "Update Copy");
    writeGame(upGame, base);
    const FEED = "https://mods.example/mod-u.json";
    const URL_NEW = "https://mods.example/mod-u-2.0.0.tcoaalmod";
    const feedFile = path.join(tmp, "feed.json");
    fs.writeFileSync(feedFile, JSON.stringify({ version: "2.0.0", url: URL_NEW }));

    const modU1 = path.join(tmp, "mod-u-1.tcoaalmod");
    const modU2 = path.join(tmp, "mod-u-2.tcoaalmod");
    fs.writeFileSync(
      modU1,
      await buildMod(base, treeA(base), fp, {
        id: "mod-u",
    ...
    // A mod with NO update source of its own: the installer has to supply one.
    const modS = path.join(tmp, "mod-s.tcoaalmod");
    fs.writeFileSync(
      modS,
      await buildMod(base, treeA(base), fp, { id: "mod-s", name: "Mod S", version: "1.0.0" }),
    );
    const upEnv = () => fetchStubEnv(tmp, { [FEED]: feedFile, [URL_NEW]: modU2 });

    run(["--install", modU1, "--game", upGame]);

    await test("an installed mod's own update source is read", () => {
      const res = runJson(["--check-update", "mod-u", "--game", upGame], { env: upEnv() });
      eq(res.updates.length, 1, "one answer");
      const u = res.updates[0];
      eq(u.current, "1.0.0", "baseline");
      eq(u.version, "2.0.0", "latest");
      eq(u.newer, true, "newer");
      eq(u.installed, true, "installed");
    });

    await test("--current raises the baseline: an installer newer than the folder is not stale", () => {
      const res = runJson(
        ["--check-update", "mod-u", "--current", "2.0.0", "--game", upGame],
        { env: upEnv() },
      );
      eq(res.updates[0].current, "2.0.0", "the carried version is the baseline");
      eq(res.updates[0].newer, false, "nothing newer than what the installer carries");
    });

    await test("--current below the installed version does not drag the baseline back", () => {
      const res = runJson(
        ["--check-update", "mod-u", "--current", "0.1.0", "--game", upGame],
        { env: upEnv() },
      );
      eq(res.updates[0].current, "1.0.0", "the newer of the two wins");
    });

    await test("--source answers for a mod whose installed copy names none", () => {
      run(["--install", modS, "--game", upGame]);
      const none = runJson(["--check-update", "mod-s", "--game", upGame], { env: upEnv() });
      eq(none.updates.length, 0, "no source means no answer");
      const res = runJson(
        [
          "--check-update",
          "mod-s",
          "--source",
          JSON.stringify({ manifest: FEED }),
          "--game",
          upGame,
        ],
        { env: upEnv() },
      );
      eq(res.updates.length, 1, "the supplied source is used");
      eq(res.updates[0].version, "2.0.0", "latest");
    });

    await test("an ONLINE source doubles as an update source", async () => {
      const onlineU = path.join(tmp, "online-u.tcoaalmod");
      fs.writeFileSync(
        onlineU,
        await buildPlaceholder({
          id: "mod-o",
          name: "Mod O",
          online: { manifest: FEED },
        }),
      );
      // What --unpack hands the app: where this installer's mod is published.
      const info = runJson(["--unpack", onlineU, "--out", path.join(tmp, "unpacked-u")]);
      eq(JSON.stringify(info.updateSource), JSON.stringify({ manifest: FEED }), "updateSource");
    });

    await test("a mod that is not installed is reported as such, not skipped", () => {
      const res = runJson(
        [
          "--check-update",
          "mod-gone",
      ...
      eq(res.updates.length, 1, "one answer");
      eq(res.updates[0].installed, false, "installed");
      eq(res.updates[0].newer, true, "newer");
    });

    await test("--apply-update installs the version the source names", () => {
      run(["--apply-update", "mod-u", "--game", upGame], { env: upEnv() });
      const st = runJson(["--status", "--game", upGame]);
      const me = st.mods.filter((m) => m.id === "mod-u")[0];
      eq(me.version, "2.0.0", "the installed version moved");
    });
  }
  ...
  // The shape every registered overhaul has: authored as a project, with its
  // own engine that opens data/System.json and img/pictures/x.png plain. The
  // package places the mod's files under the mod's names; what the game
  // already holds is laid down from the player's own copy (copy entries, and
  // patches that replay on the base's storage name) so the package carries
  // none of it. The profile has to come out as the player's game with the
  // mod's folder pasted on top.
  await test("a project-space mod installs as itself over the player's game", async () => {
    const pjGame = path.join(tmp, "Project Game");
    const pjBase = baseTree();
    const keep = await C.hashPath("audio/bgm/keep.ogg");
    const kept = new Uint8Array([10, 20, 30, 40, 50]);
    pjBase.set(keep, C.enkit(kept, keep, 0));
    writeGame(pjGame, pjBase);
    const pjWww = path.join(pjGame, "www");
    const gameInos = new Map(
      [...readTree(pjWww).keys()].map((rel) => [rel, ino(path.join(pjWww, rel))]),
    );

    const sys = JSON.parse(dec(C.dekit(pjBase.get(SYSTEM), SYSTEM)));
    sys.gameTitle = "TCOAAL (project)";
    const project = new Map([
      ["data/System.json", enc(JSON.stringify(sys))], // patch, from the base's name
      ["audio/bgm/keep.ogg", kept], // copy: the player's own track
      ["audio/bgm/renamed.ogg", kept], // copy: the same track, found by content
      ["img/pictures/mine.png", new Uint8Array([7, 7, 7])], // verbatim
      ["js/plugins/GameCode.js", enc("App.usesObfuscation = function () { return false; };\n")],
    ]);
    const storage = await D.storageNames([...project.keys()]);
    const payloads = new Map();
    const r = await D.compare(D.memSource(pjBase), D.memSource(project), payloads, () => {}, {
      mode: "overlay",
      storage,
    });
    eq(r.stats.copied, 2, "copied");
    eq(r.stats.patched, 1, "patched");
    eq(r.files.find((f) => f.rel === "data/System.json").from, SYSTEM);
    const pjFp = await D.fingerprint(D.memSource(pjBase));
    const pjMod = path.join(tmp, "project.tcoaalmod");
    fs.writeFileSync(
      pjMod,
      await P.build({
        id: "mod-pj", name: "Project", author: "kidev", version: "1.0.0", description: "",
        icon: null, saves: "isolated", update: null, theme: [], payloads,
        variants: [{ base: { label: "v3.0.8", fingerprint: pjFp }, files: r.files, stats: r.stats }],
      }),
    );
    const { manifest } = await P.parse(new Uint8Array(fs.readFileSync(pjMod)));
    eq(manifest.format, "tcoaal-mod/4");

    run(["--install", pjMod, "--game", pjGame]);
    const res = runJson(["--profile", "--game", pjGame]);
    assert(res.profile.built, "no profile was built");
    const pjProfileWww = path.join(pjGame, "addons", ".profile", "www");
    const tree = readTree(pjProfileWww);

    // Every file of the mod's, under the mod's name, with the mod's bytes.
    for (const [rel, want] of project) {
      assert(tree.has(rel), "the profile is missing the mod's " + rel);
      if (rel === "data/System.json") {
        eq(JSON.stringify(JSON.parse(dec(tree.get(rel)))), JSON.stringify(sys), "the patched document, plain");
      } else {
        eq(hex(tree.get(rel)), hex(want), rel + " bytes");
      }
      assert(
        !gameInos.has(rel) || ino(path.join(pjProfileWww, rel)) !== gameInos.get(rel),
        rel + " was written through a link into the player's game",
      );
    }
    // The player's own files are still there, still theirs.
    for (const [rel, before] of gameInos) {
      if (SKIP_IN_PROFILE.test(rel)) continue;
      assert(tree.has(rel), "the profile lost the game's " + rel);
      eq(ino(path.join(pjProfileWww, rel)), before, rel + " is no longer the player's own file");
    }
    // And the game folder itself is untouched, byte for byte.
    const now = readTree(pjWww);
    for (const [rel, bytes] of pjBase) {
      eq(hex(now.get(rel)), hex(bytes), "the game's " + rel + " was modified");
    }
