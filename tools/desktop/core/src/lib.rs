...
/// Whether the Steam client is running, as far as this machine can say.
///
/// `None` means the question could not be answered here, which is not the same
/// as "no" and must never be treated as one.
///
/// The game asks this before it draws anything. `SceneManager.run` in its DRM
/// payload calls `Steam.init()` first and simply RETURNS when that fails: no
/// scene, no message, no log line beyond one the player never sees - a black
/// window that looks for all the world like the mod broke the game. Steam is
/// running for a player who started the game from their library, so the base
/// game never meets this; a mod launched from here is the one case where it
/// can be missing, so the launcher has to say so itself.
pub fn steam_running() -> Option<bool> {
    #[cfg(target_os = "linux")]
    {
        // Steam keeps a pid file next to its root. Trusting it alone would
        // report a crashed client as running, so the pid has to still be a
        // live process.
        let mut sawfile = false;
        if let Some(home) = home_dir() {
            for rel in [
                ".steam/steam.pid",
                ".var/app/com.valvesoftware.Steam/.steam/steam.pid",
                "snap/steam/common/.steam/steam.pid",
            ] {
        ...
        // No pid file, or a stale one: read the process table directly rather
        // than guess. A client started in a way that writes no pid file is
        // still a client that is running.
        let Ok(entries) = fs::read_dir("/proc") else {
            return if sawfile { Some(false) } else { None };
        };
        for e in entries.flatten() {
    ...
    {
        // The client publishes its own pid here and zeroes it on exit.
        ...
        let Ok(out) = no_console(&mut cmd).output() else {
            return None;
        };
        ...
        let value = text
            .lines()
            .find_map(|l| l.split_whitespace().last().filter(|v| v.starts_with("0x")))?;
        let pid = u32::from_str_radix(value.trim_start_matches("0x"), 16).ok()?;
        return Some(pid != 0);
    }
    #[cfg(target_os = "macos")]
    {
        let mut cmd = Command::new("pgrep");
        cmd.args(["-x", "steam_osx"]);
        let Ok(out) = no_console(&mut cmd).output() else {
            return None;
        };
...
pub struct ProtonRun {
    ...
    /// The Steam Linux Runtime command that must WRAP this runner, already
    /// resolved to an argv prefix (entry point first). Empty when the runner
    /// names no runtime, or the runtime it names is not installed.
    pub container: Vec<String>,
    /// The verb to pass (to the container and to proton). `run` bare,
    /// `waitforexitandrun` inside a container, which is what Steam passes.
    pub verb: String,
}

/// The `installdir` of an installed app, looked up across every library.
fn installed_app_dir(libraries: &[String], appid: &str) -> Option<PathBuf> {
    for lib in libraries {
        ...
        if let Some(dir) = kv_after(&text, "\"installdir\"") {
...
/// The Steam Linux Runtime command that a compatibility tool has to be run
/// inside, as an argv prefix, or `None` when it names none (or it is not
/// installed).
///
/// This is the container Steam itself puts every Proton game in, and running
/// without it is running the game against whatever the player's distribution
/// happens to have. That is not a theoretical difference: the game here is a
/// 32-bit Windows build, so wine loads 32-bit audio libraries for it, and a
/// 64-bit-only host (an ordinary Arch install, say) has no 32-bit libpulse at
/// all. Wine then falls back to ALSA, ALSA's PulseAudio plugin cannot load
/// either, and the game plays every sound into a dead device: no music, no
/// voices, no error anywhere, because nothing actually failed. The container
/// carries a complete 32- and 64-bit library set, which is exactly why Steam
/// uses it, and the same reasoning covers everything else the game may reach
/// for at runtime rather than at load time.
///
/// Resolved the way Steam resolves it, not by hardcoding a name: the tool's
/// `toolmanifest.vdf` names the runtime's app id in `require_tool_appid`, and
/// the runtime's own manifest gives the command line to invoke it with
/// (`/_v2-entry-point --verb=%verb% --`).
fn runtime_container(tool_dir: &Path, libraries: &[String], verb: &str) -> Option<Vec<String>> {
    let manifest = fs::read_to_string(tool_dir.join("toolmanifest.vdf")).ok()?;
    let appid = kv_after(&manifest, "\"require_tool_appid\"")?;
    if appid.trim().is_empty() {
        return None;
    }
    let runtime_dir = installed_app_dir(libraries, appid.trim())?;
    let runtime_manifest = fs::read_to_string(runtime_dir.join("toolmanifest.vdf")).ok()?;
    let commandline = kv_after(&runtime_manifest, "\"commandline\"")?;
    let mut parts = commandline.split_whitespace();
    // The first token is the entry point, written relative to the runtime
    // directory with a leading slash.
    let entry = runtime_dir.join(parts.next()?.trim_start_matches('/'));
    if !entry.is_file() {
        return None;
    }
    let mut argv = vec![entry.to_string_lossy().into_owned()];
    for arg in parts {
        argv.push(arg.replace("%verb%", verb));
    }
    Some(argv)
}
...
pub fn find_protons(appid: &str) -> Option<Vec<ProtonRun>> {
    ...
    Some(
        candidates
            .into_iter()
            .map(|p| {
                // Inside the container the verb is the one Steam passes;
                // without it, the bare `run` verb this has always used.
                let container = runtime_container(&p, &steam.libraries, "waitforexitandrun");
                ProtonRun {
                    ...
                    verb: if container.is_some() { "waitforexitandrun" } else { "run" }.into(),
