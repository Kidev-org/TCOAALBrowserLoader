...
use tcoaal_desktop_core::{
    ...
    shared_data_dir, steam_running, GameLocation, ToolResult, TCOAAL_APPID,
};
...
const NODE_ZST: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/node.zst"));
...
fn stage_resources(parent: &Path) -> Result<PathBuf, String> {
    ...
    let mut zst = zstd::Decoder::new(NODE_ZST)
        .map_err(|e| format!("Cannot read the Node runtime: {e}"))?;
    std::io::copy(&mut zst, &mut out)
        .map_err(|e| format!("Cannot unpack the Node runtime: {e}"))?;
...
/// The shape of the unpacked backend, bumped when a build changes what lives
/// in the app's cache directory or how it is laid out. Not when the mod or
/// the app version changes, which the rest of the marker already covers.
///
/// Every stamped installer on a machine shares one such folder, so they do not
/// all agree about it: the one that upgrades it is the newest generation that
/// runs, and older ones then leave it as they find it.
const BACKEND_GENERATION: u32 = 1;

/// The generation recorded in a marker line, or 0 for a marker written before
/// there were generations (and for an unreadable or missing one), which is
/// older than every real generation and so gets upgraded.
fn marker_generation(line: &str) -> u32 {
...
fn housekeeping() {
    ...
    let had_gen = marker_generation(&have);

    // A NEWER installer taking over this folder rewrites the whole of it: the
    // layout it wants may not be the layout that is there, and everything in
    // here is derived, so the cheapest correct answer is to start again.
    // An OLDER one must not do the same in reverse. A player who keeps two
    // stamped installers would otherwise have them drag the shared folder back
    // and forth, each run throwing away a hundred megabytes the other just
    // unpacked, so it leaves the folder alone and stages its own
    // content-named directory beside what is already there.
    if had_gen > BACKEND_GENERATION {
        return;
    }
...
/// What the game is doing, forwarded to the webview so the screen can stop
/// saying "starting" once it has started, and say something again when the
/// player closes it and comes back to a window that has been sitting on the
/// same sentence the whole time. Payload: `{ "state": "running" | "exited",
/// "code": <exit code or null>, "watched": <bool> }`.
const GAME_EVENT: &str = "game-state";
...
/// Look for a newer release of a mod.
///
/// `current` and `source` are what this BINARY knows and the game folder does
/// not: the version of the mod stamped into it, and where that mod is
/// published. Without them an installer could only ever check a mod that was
/// already installed and whose author had filled in a separate update field,
/// which is why the check never fired for anybody.
#[tauri::command]
async fn check_update(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        if let Some(src) = source.filter(|v| !v.is_null()) {
...
async fn apply_update(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let mut args = vec!["--apply-update".into(), id];
        if let Some(src) = source.filter(|v| !v.is_null()) {
            args.push("--source".into());
            args.push(src.to_string());
        }
...
/// Whether Steam is up, for a screen that would otherwise offer a Play button
/// that cannot work. `null` is "could not tell", and a page must read it as
/// yes: see `core::steam_running`.
#[tauri::command]
async fn steam_state() -> Result<Value, String> {
    Ok(json!({ "running": steam_running() }))
}

/// The name, icon and screen of a mod that is installed in `root` but is not
/// the one this binary carries, so the app can wear a different mod's face
/// when the player switches to it. Read straight out of `addons/<id>/`, which
/// is where mod-loader unpacked exactly these three things at install time.
#[tauri::command]
async fn mod_face(root: String, id: String) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || {
        // The id comes from a list this app produced, but it still lands in a
        // path: anything that could climb out of addons/ is refused here
        // rather than trusted to have come from our own UI.
        if id.is_empty()
            || !id
                .chars()
            ...
            || id.starts_with('.')
        {
            return Err(format!("Not a mod id: {id}"));
        }
        let dir = Path::new(&root).join("addons").join(&id);
        if !dir.join("mod.json").is_file() {
            return Err(format!("\"{id}\" is not installed here."));
        }
        let raw = fs::read_to_string(dir.join("mod.json")).map_err(|e| e.to_string())?;
        ...
        Ok(json!({ "mod": manifest, "theme": read_theme(&dir) }))
    })
    .await
    .map_err(|e| e.to_string())?
}
...
///
/// A launch that survives the grace period is then WATCHED rather than
/// forgotten: the child is kept on a thread that waits for it and reports the
/// exit, so the window can say the game is running while it runs and stop
...
fn start_and_watch(
    ...
    // launch fails. The pipes are dropped once the launch has taken, which
    // leaves the game writing to a closed stdout, which it does not read
    // back and does not care about. Holding them open while nobody reads
    // would stop the game dead as soon as the pipe filled.
    ...
    match settled(&mut child, LAUNCH_GRACE) {
        None => {
            ...
            let _ = app.emit(GAME_EVENT, json!({ "state": "running", "watched": true }));
            let handle = app.clone();
            std::thread::spawn(move || {
                let code = match child.wait() {
                    Ok(status) => status.code().map(Value::from).unwrap_or(Value::Null),
                    // Reaped by something else, or never waitable: the game is
                    // no longer ours to watch, which for the screen is the same
                    // news as an exit.
                    Err(_) => Value::Null,
                };
                let _ = handle.emit(GAME_EVENT, json!({ "state": "exited", "code": code }));
            });
            Ok(())
        }
        Some(status) => {
...
/// What to tell a player whose Steam client is not running.
///
/// The game will not draw a single frame without it, and it says so to nobody:
/// see core::steam_running. A refusal here with a sentence they can act on is
/// the whole difference between that and a black window.
const STEAM_NOT_RUNNING: &str = "Steam is not running.\n\n\
     The game checks for Steam before it starts, and with Steam closed it \
     opens a black window and stops there. It will not tell you why. Start \
     Steam, wait for it to sign in, then press Play again.";

fn spawn_game(app: &AppHandle, exe: &Path) -> Result<(), String> {
    use std::process::Command;
    let dir = exe.parent().unwrap_or(Path::new("."));
    let kind = program_kind(exe);

    // Only a positive "no" stops the launch. A machine this cannot be answered
    // on (steam_running returns None) gets the benefit of the doubt: a player
    // kept from their game by a check that failed is worse off than one who
    // sees the black window this is here to prevent.
    if steam_running() == Some(false) {
        return Err(STEAM_NOT_RUNNING.to_string());
    }
    ...
    if cfg!(target_os = "macos") && exe.extension().is_some_and(|e| e == "app") {
        ...
        if out.status.success() {
            // Launch Services owns the game now, so there is no exit to wait
            // for and the screen is told not to expect one.
            let _ = app.emit(GAME_EVENT, json!({ "state": "running", "watched": false }));
            return Ok(());
        }
        return Err(format!(
    ...
    if kind == Program::Windows && !cfg!(target_os = "windows") {
        ...
        // once. Only the first failure is worth reporting, since it is the
        // runner the player's Steam would have used, so that is the one kept.
        ...
        //
        // SteamEnv=1 is what the Steam client stamps on every game it starts
        // itself, and it has to be here too. Proton wraps the game in its
        // steam.exe helper, which calls SteamAPI_Init before it spawns the
        // game; for a process the client did not start, the client's
        // steamclient.so answers that by applying "the Steam environment for
        // the app" to the process (the same set it gives its own launches,
        // LC_ALL=C included), and the game inherits it. Wine's ntdll reads
        // the Unix code page from LC_CTYPE at startup, LC_ALL=C makes that
        // plain ASCII (WINEUNIXCP=20127), and from then on no file with a
        // non-ASCII name exists as far as the game can tell: a mod's
        // img/tilesets/<Cyrillic>.png fails to load while the file sits right
        // there in the profile. With SteamEnv already set the client treats
        // the session as its own and leaves the environment alone, exactly as
        // it does when Steam launches the game, and the code page follows
        // LANG (UTF-8). The Steam API still initialises: the app id travels
        // in SteamAppId and steam_appid.txt.
        //
        // The runner is WRAPPED in its Steam Linux Runtime when it names one
        // (core::find_protons resolves it), because that container is the
        // library environment Steam gives every Proton game and the game is
        // built for it. Running Proton bare hands the game the host's
        // libraries instead, and the host is under no obligation to have the
        // 32-bit half of anything: on a machine with no 32-bit libpulse, wine
        // finds no PulseAudio driver for this 32-bit game, falls back to ALSA,
        // ALSA's own pulse plugin fails to load for the same reason, and every
        // sound the game plays goes into a device that is not there. It runs
        // perfectly and in total silence, with nothing reported anywhere.
        // Sound is just the first thing that broke; the fix is to stop
        // improvising an environment and use the one Steam uses.
        let mut first_error = None;
        for run in runs {
            let mut cmd = match run.container.split_first() {
                Some((entry, args)) => {
                    let mut cmd = Command::new(entry);
                    clean_child_env(&mut cmd);
                    cmd.args(args).arg(&run.proton);
                    cmd
                }
                None => {
            ...
            cmd.arg(&run.verb)
                ...
                // What the container has to make visible from the host: the
                // profile is a game root of its own, and a copy of the game
                // outside Steam's own libraries would otherwise not be there
                // to run.
                .env("STEAM_COMPAT_INSTALL_PATH", dir)
                .env("STEAM_COMPAT_MOUNTS", dir)
                .env("SteamAppId", TCOAAL_APPID)
                .env("SteamGameId", TCOAAL_APPID)
                .env("SteamEnv", "1");
            match start_and_watch(app, cmd, dir) {
                Ok(()) => return Ok(()),
